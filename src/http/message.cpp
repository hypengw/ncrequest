    ...
    RSTD_MATCH(rstd::move(parsed)) {
        RSTD_CASE(Request, value) {
            auto method_text = ref<str>::from_raw_parts(
                input.as_raw_ptr() + value.method.begin, value.method.size());
            ...
            auto target_text = ref<str>::from_raw_parts(
                input.as_raw_ptr() + value.target.begin, value.target.size());
            return Ok(StartLine::Request(RequestLine {
                rstd::move(method).unwrap(), String::make(target_text),
        ...
        RSTD_CASE(Response, value) {
            ...
            auto reason = HeaderValue::from_bytes(slice<u8>::from_raw_parts(
                input.as_raw_ptr() + value.reason.begin, value.reason.size()));
            if (reason.is_err()) {
            ...
            return Ok(StartLine::Response(StatusLine {
                Some(Version { value.major, value.minor }), rstd::move(status).unwrap(),
...
auto make_field(slice<u8> input, parser::http1::FieldLine parsed, usize base)
    ...
    auto name_text = ref<str>::from_raw_parts(
        input.as_raw_ptr() + parsed.name.begin, parsed.name.size());
    ...
    auto value = HeaderValue::from_bytes(slice<u8>::from_raw_parts(
        input.as_raw_ptr() + parsed.value.begin, parsed.value.size()));
    if (value.is_err()) {
        return Err(HttpParseError { HttpParseErrorKind::InvalidHeaderLine(),
...
auto Method::parse(ref<str> input) -> rstd::Result<Method, HttpParseError> {
    if (input.size() == 0) {
        return Err(HttpParseError { HttpParseErrorKind::InvalidStartLine(), 0 });
    }
...
auto Version::parse(ref<str> input) -> rstd::Result<Version, HttpParseError> {
    if (input.size() != 8 || input.data()[0] != 'H' || input.data()[1] != 'T' ||
        input.data()[2] != 'T' || input.data()[3] != 'P' || input.data()[4] != '/' ||
        ! parser::ascii::digit(input.data()[5]) || input.data()[6] != '.' ||
        ! parser::ascii::digit(input.data()[7])) {
        return Err(HttpParseError { HttpParseErrorKind::InvalidStartLine(), 0 });
    }
    return Ok(Version { static_cast<u8>(input.data()[5] - '0'),
                        static_cast<u8>(input.data()[7] - '0') });
}
...
auto StatusCode::parse(ref<str> input) -> rstd::Result<StatusCode, HttpParseError> {
    ...
    u16 value = 0;
    for (usize offset = 0; offset < input.size(); ++offset) {
        if (! parser::ascii::digit(input.data()[offset])) {
            return Err(HttpParseError { HttpParseErrorKind::InvalidStartLine(), offset });
        }
        value = static_cast<u16>(value * 10 + input.data()[offset] - '0');
    }
    return make(value);
}
...
auto MessageHead::parse(slice<u8> input) -> rstd::Result<MessageHead, HttpParseError> {
    ...
    if (event.is_Complete()) {
        ...
        if (complete.consumed != input.len()) {
            return Err(HttpParseError { HttpParseErrorKind::InvalidSyntax(),
                                        complete.consumed });
        }
...
auto Http1HeadParser::push(slice<u8> input)
    -> rstd::Result<Http1HeadEvent, HttpParseError> {
    if (complete_) {
        return Err(HttpParseError { HttpParseErrorKind::InvalidSyntax(), line_start_ });
    }
    buffer_.extend_from_slice(input);

    while (scan_ + 1 < buffer_.len()) {
        if (buffer_[scan_] != '\r' || buffer_[scan_ + 1] != '\n') {
            ++scan_;
            continue;
        }

        auto line_end = scan_ + 2;
        if (line_end > MaxHeaderBytes) {
            return Err(HttpParseError { HttpParseErrorKind::HeaderTooLarge(),
                                        MaxHeaderBytes });
        }
        auto line = slice<u8>::from_raw_parts(
            buffer_.as_slice().as_raw_ptr() + line_start_, line_end - line_start_);
