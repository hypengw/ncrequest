...
auto equals_ascii_case_insensitive(ref<str> left, const char* right) noexcept -> bool {
    auto right_size = rstd::strlen(right);
    if (left.size() != right_size) return false;

    for (usize i = 0; i < left.size(); ++i) {
        auto lhs = static_cast<unsigned char>(left.data()[i]);
        auto rhs = static_cast<unsigned char>(right[i]);
...
auto starts_with_at(ref<str> input, usize offset, ref<str> prefix) noexcept -> bool {
    ...
    for (usize i = 0; i < prefix.size(); ++i) {
        if (input.data()[offset + i] != prefix.data()[i]) return false;
    }
    return true;
}

void append_range(String& output, ref<str> input, usize begin, usize end) {
    output.push_str(ref<str>::from_raw_parts(input.data() + begin, end - begin));
}
...
auto remove_dot_segments(ref<str> input) -> String {
    ...
    while (offset < input.size()) {
        ...
        if (starts_with_at(input, offset, "/.") && offset + 2 == input.size()) {
            output.push_back('/');
            break;
        }
        if (starts_with_at(input, offset, "/../")) {
        ...
        if (starts_with_at(input, offset, "/..") && offset + 3 == input.size()) {
            remove_last_segment(output);
            output.push_back('/');
            break;
        }
        if ((starts_with_at(input, offset, ".") && offset + 1 == input.size()) ||
            (starts_with_at(input, offset, "..") && offset + 2 == input.size())) {
            break;
        }

        auto segment_end = offset;
        if (input.data()[segment_end] == '/') ++segment_end;
        while (segment_end < input.size() && input.data()[segment_end] != '/') ++segment_end;
...
auto merge_paths(const Url& base, ref<str> reference_path) -> String {
    ...
    if (base.authority().is_some() && base_path.size() == 0) {
        merged.push_back('/');
    ...
    while (prefix > 0 && base_path.data()[prefix - 1] != '/') --prefix;
    append_range(merged, base_path, 0, prefix);
    merged.push_str(reference_path);
    return merged;
}
...
auto parser_error(parser::ParseFailure failure) -> UrlError {
    ...
    if (expected.is_IpLiteral() || expected.is_Ipv4Address() ||
        expected.is_Ipv6Address()) {
...
Url::Url(String source, Component scheme, Component authority, Component userinfo,
...
auto Url::parse(ref<str> input) -> rstd::Result<Url, UrlError> {
    ...
    return Ok(Url { String::make(input), optional(parts.scheme), optional(parts.authority),
                    optional(parts.userinfo), optional(parts.host), optional(parts.port), path,
                    optional(parts.query), optional(parts.fragment) });
}
...
auto Url::parse_http(ref<str> input) -> rstd::Result<Url, UrlError> {
    ...
    if (scheme_ref.is_none()) {
        return Err(UrlError { UrlErrorKind::MissingScheme(), 0 });
    }
    if (! equals_ascii_case_insensitive(*scheme_ref, "http") &&
        ! equals_ascii_case_insensitive(*scheme_ref, "https")) {
        return Err(UrlError { UrlErrorKind::UnsupportedScheme(), 0 });
    }
    if (url.authority().is_none()) {
        return Err(UrlError { UrlErrorKind::MissingAuthority(), scheme_ref->size() + 1 });
    }
    auto host_ref = url.host();
    if (host_ref.is_none() || host_ref->size() == 0) {
        auto authority_ref = url.authority();
        auto offset = authority_ref.is_some()
                          ? static_cast<usize>(authority_ref->data() - url.as_ref().data())
                          : 0;
        return Err(UrlError { UrlErrorKind::MissingHost(), offset });
    }

    auto host_value = *host_ref;
    if (host_value.size() > 0 && host_value.data()[0] != '[') {
        bool numeric = false;
        bool dotted  = false;
        ...
        if (numeric && dotted) {
            ...
            if (ipv4.is_err()) {
                ...
                auto offset = static_cast<usize>(host_value.data() - url.as_ref().data()) +
                              failure.offset();
    ...
    if (port_ref.is_some() && port_ref->size() > 0) {
        usize value = 0;
        for (usize i = 0; i < port_ref->size(); ++i) {
            value = value * 10 + static_cast<usize>(port_ref->data()[i] - '0');
            if (value > 65535) {
                auto offset = static_cast<usize>(port_ref->data() - url.as_ref().data()) + i;
...
auto Url::component(Component value) const noexcept -> Option<ref<str>> {
    ...
    return Some(ref<str>::from_raw_parts(source.data() + value.offset, value.size));
}
...
auto Url::path() const noexcept -> ref<str> {
    auto source = source_.as_str();
    return ref<str>::from_raw_parts(source.data() + path_.offset, path_.size);
}
...
auto Url::request_target() const -> String {
    ...
    auto path_view = path();
    if (path_view.size() == 0 && authority_.present) {
        target.push_back('/');
    } else {
...
auto Url::clone() const -> Url {
    return Url { source_.clone(), scheme_, authority_, userinfo_, host_, port_, path_, query_,
                 fragment_ };
}
