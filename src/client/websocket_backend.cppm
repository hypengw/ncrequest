...
concept WebSocketBackend =
    requires(T client, const T const_client, const std::string& url, std::string_view text,
             std::span<const rstd::byte> bytes, typename T::ConnectedCallback connected,
        ...
        { client.set_on_disconnected_callback(rstd::move(disconnected)) } -> std::same_as<void>;
